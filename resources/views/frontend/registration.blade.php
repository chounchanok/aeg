...
<body>
    ...
    <div class="registration-card">
        <h1 class="form-title">{{ __('ลงทะเบียน') }}</h1>
        <p class="form-subtitle">{{ __('กรอกเบอร์มือถือของคุณเพื่อรับรหัส OTP') }}</p>
        ...
        @csrf
            <div class="phone-input-group">
                ...
                <input type="tel" name="phone" class="form-control phone-input-field" value="{{ old('phone') }}" placeholder="{{ __('เบอร์มือถือ') }}" required>
            </div>
            @error('phone')
                <div class="text-danger mt-1 text-sm">{{ $message }}</div>
            @enderror

            <button type="submit" class="btn btn-otp-custom">{{ __('ขอรหัส OTP') }}</button>
        </form>
