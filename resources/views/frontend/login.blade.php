...
<body>
    ...
    <div class="login-card">
        ...
        <div class="social-icons-group">
            ...
            <!-- WhatsApp (เปลี่ยนเป็นเปิด Modal) -->
            <button type="button" data-bs-toggle="modal" data-bs-target="#whatsappModal" class="social-login-btn text-decoration-none" title="เข้าสู่ระบบด้วย WhatsApp">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius: 20px;">
                <div class="modal-body p-4 text-center">
                    <svg viewBox="0 0 448 512" fill="#25D366" width="60" class="mb-3"><path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm..."></path></svg>
                    <h4 class="fw-bold mb-3">WhatsApp Login</h4>

                    <!-- Step 1: ขอ OTP -->
                    <div id="wa-step-1">
                        <p class="text-muted mb-4">กรุณากรอกเบอร์โทรศัพท์เพื่อรับรหัส OTP ทาง WhatsApp</p>
                        <input type="tel" id="wa-phone" class="form-control form-control-lg mb-3 text-center" placeholder="08X-XXX-XXXX">
                        <button type="button" class="btn btn-login-custom m-0" onclick="requestWaOtp()" id="btnWaRequest">ขอรหัส OTP</button>
                    </div>

                    <!-- Step 2: กรอก OTP -->
                    <div id="wa-step-2" style="display: none;">
                        <p class="text-muted mb-4">กรอกรหัส 6 หลักที่ได้รับทางแชท WhatsApp ของคุณ</p>
                        <input type="number" id="wa-otp" class="form-control form-control-lg mb-3 text-center" placeholder="------" style="letter-spacing: 5px;">
                        <button type="button" class="btn btn-login-custom m-0" onclick="verifyWaOtp()" id="btnWaVerify">ยืนยันเข้าสู่ระบบ</button>
                        <button type="button" class="btn btn-link text-muted mt-3" onclick="resetWaModal()">เปลี่ยนเบอร์โทรศัพท์</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        function requestWaOtp() {
            const phone = document.getElementById('wa-phone').value;
            const btn = document.getElementById('btnWaRequest');
            if(!phone) return alert('กรุณากรอกเบอร์โทรศัพท์');

            btn.disabled = true;
            btn.innerHTML = 'กำลังส่ง...';

            fetch('{{ route("whatsapp.request-otp") }}', {
                method: 'POST',
        ...
        function verifyWaOtp() {
            const phone = document.getElementById('wa-phone').value;
            const code = document.getElementById('wa-otp').value;
            const btn = document.getElementById('btnWaVerify');
            if(!code) return alert('กรุณากรอกรหัส OTP');
            ...
            fetch('{{ route("whatsapp.verify-otp") }}', {
                ...
                if(data.status === 'success') {
                    window.location.href = '{{ route("home") }}'; // สำเร็จแล้วกลับหน้าแรก
                } else {
                    btn.disabled = false;
                    btn.innerHTML = 'ยืนยันเข้าสู่ระบบ';
                    alert(data.message);
                }
            });
        }
        ...
        function resetWaModal() {
            ...
            document.getElementById('wa-otp').value = '';
        }
    </script>
    <!-- Bootstrap 5 JS -->
