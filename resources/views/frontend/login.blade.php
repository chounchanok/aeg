...
<body>
    ...
    <div class="login-card">
        <h1 class="form-title">{{ __('เข้าสู่ระบบ') }}</h1>
        <p class="form-subtitle">{{ __('ยินดีต้อนรับกลับมา! กรุณาเข้าสู่ระบบ') }}</p>

        @if (session('error'))
            <div class="alert alert-danger alert-dismissible fade show" role="alert" style="border-radius: 12px;">
                <strong>{{ __('เกิดข้อผิดพลาด!') }}</strong> {{ session('error') }}
        ...
        @csrf
            ...
            <div class="mb-2">
                <label class="input-label">{{ __('รหัสผ่าน') }}</label>
                <input type="password" name="password" class="form-control input-field" placeholder="{{ __('กรอกรหัสผ่านของคุณ') }}" required>
                @error('password')
                    <div class="text-danger mt-1 text-sm">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="forgot-password">
                <a href="{{ route('forgot-password') }}">{{ __('ลืมรหัสผ่าน?') }}</a>
            </div>

            <button type="submit" class="btn btn-login-custom">{{ __('เข้าสู่ระบบ') }}</button>
        </form>

        <div class="register-link">
            {{ __('ยังไม่มีบัญชี?') }} <a href="{{ route('register') }}">{{ __('สมัครสมาชิก') }}</a>
        </div>

        <div class="social-divider">
            <span>{{ __('หรือเข้าสู่ระบบด้วย') }}</span>
        </div>

        <div class="social-icons-group row justify-content-center g-3">
            <!-- LINE -->
            <a href="{{ route('social.redirect', 'line') }}" class="social-login-btn text-decoration-none col-lg-3 col-sm-6" title="{{ __('เข้าสู่ระบบด้วย') }} LINE">
                <img src="https://upload.wikimedia.org/wikipedia/commons/4/41/LINE_logo.svg" alt="LINE Icon">
            </a>
            
            <!-- Google -->
            <a href="{{ route('social.redirect', 'google') }}" class="social-login-btn text-decoration-none col-lg-3 col-sm-6" title="{{ __('เข้าสู่ระบบด้วย') }} Google">
            ...
            <!-- Facebook -->
            <a href="{{ route('social.redirect', 'facebook') }}" class="social-login-btn text-decoration-none col-lg-3 col-sm-6" title="{{ __('เข้าสู่ระบบด้วย') }} Facebook">
            ...
            <button type="button" data-bs-toggle="modal" data-bs-target="#whatsappQrModal" onclick="initWhatsAppLogin()" class="social-login-btn text-decoration-none col-lg-3 col-sm-6" title="{{ __('เข้าสู่ระบบด้วย') }} WhatsApp">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius: 20px;">
                <div class="modal-body p-4 text-center">
                    ...
                    <h4 class="fw-bold mb-2">{{ __('เข้าสู่ระบบด้วย') }} WhatsApp</h4>
                    <p class="text-muted mb-4" style="font-size: 0.95rem;">{{ __('สแกน QR Code ด้านล่างเพื่อส่งข้อความยืนยัน') }}<br>{{ __('ระบบจะเข้าสู่ระบบให้อัตโนมัติ') }}</p>
                    ...
                    <p class="text-sm text-secondary mb-0">{{ __('กำลังรอการยืนยัน...') }}</p>
                    <p class="text-xs text-muted mt-1 mb-0" style="font-size: 0.75rem;">({{ __('รหัสอ้างอิง:') }} <span id="wa-login-code">---</span>)</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function initWhatsAppLogin() {
            ...
            // ⚠️ ใส่เบอร์ WhatsApp ของบริษัท
            const waNumber = '66617693645';
